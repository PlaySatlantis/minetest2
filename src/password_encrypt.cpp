...
static uint8_t encryption_key[AES_BLOCKLEN] = {0x2b, 0x7e, 0x16, 0x16, 0x28, 0xae, 0xd2, 0xa6, 0xab, 0xf7, 0x15, 0x88, 0x09, 0xcf, 0x4f, 0x3c};
static uint8_t iv[AES_BLOCKLEN] = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f};
static struct AES_ctx ctx;
static const char *password_file_path = "userdata.bin";

constexpr std::size_t buffer_len = AES_BLOCKLEN * 8;

static_assert(buffer_len == 128);
...
bool setUserPassword(const char *password)
{
    ...
    assert(password_len < buffer_len);
    ...
    AES_CBC_encrypt_buffer(&ctx, reinterpret_cast<uint8_t*>(buffer), buffer_len);
    ...
    printf("Decrypt test..\n");
    AES_ctx_set_iv(&ctx, iv);
    AES_CBC_decrypt_buffer(&ctx, reinterpret_cast<uint8_t*>(buffer), buffer_len);
